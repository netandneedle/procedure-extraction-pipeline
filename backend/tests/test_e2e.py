...
async def _dispose_engine_pool_between_tests():
    ...
    See the matching fixture in the repo-root tests/test_e2e.py for the
    full rationale: module-level async engine + pytest-asyncio per-test
    event loops cause "Future attached to a different loop" when a pooled
    asyncpg connection is reused across loops. Disposing after each test
    (in its own loop) gives the next test fresh, correctly-bound
    connections. No-op for LLM-mocked tests that never use the pool.
    """
    yield
...
def _mock_techniques(chunk_ids: list[str]):
    # TechniqueItem requires `rationale` — was omitted before Pydantic validation.
    chunk_techniques = []
    technique_data = [
    ...
    for i, cid in enumerate(chunk_ids):
        tid, tname, tactic, conf, rationale = technique_data[i]
...
async def compiled_graph(checkpointer):
    ...
    return compile_pipeline(
        ...
        interrupt_before=["gate_0", "gate_1", "gate_2"],
    )
...
class TestE2EPipeline:
    ...
    @pytest.mark.skip(reason="Predates the propose+pick mock shape; the repo-root tests/test_e2e.py covers the same path with the current mocks.")
    @pytest.mark.asyncio
    async def test_full_pipeline_gates_disabled(
        ...
        def _universal_llm_mock(*args, **kwargs):
            ...
            if tool_name == "extract_entities":
            ...
            elif tool_name == "extract_techniques":
                # Parse chunk IDs from the prompt message
                ...
                return _mock_techniques(chunk_ids[:3] if chunk_ids else ["chk-001", "chk-002", "chk-003"])
